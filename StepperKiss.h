// Classe StepperKiss : driver pas‑à‑pas sans à‑coups (adapté de la version ESP8266).
// Ce fichier est copié presque tel quel depuis le projet original.
// Il fonctionne sur toute carte Arduino, notamment le RP2040 (Raspberry Pi Pico).
// - Init : configure les broches, vitesse max, accélération et activation.
// - Commandes : moveTo(), move(), stop(), emergencyStop().
// - run() : planifie et exécute au plus un pas par appel.
// - Gestion douce accel/décel et I/O rapides sur ESP8266.
...
// - Planificateur à micros() : un seul pas max par run()
...
public:
  ...
  void begin(uint8_t stepPin, uint8_t dirPin, int8_t enaPin = 1, bool enaActiveLow = true) {
    ...
    _stepIntervalUs = 1000;   // valeur sûre temporaire
  }

  // Rend les valeurs de vitesse et accélération noob proof (valeurs négatives impossible)
